...
import show.trom.mod.client.animation.definitions.CrystalSkeletonAnimation;
import show.trom.mod.entity.CrystalSkeleton;

public class CrystalSkeletonModel<T extends CrystalSkeleton> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(TromMod.MOD_ID, "crystal_skeleton"), "main");
    public final ModelPart root;
    private final ModelPart body;
    private final ModelPart right_leg;
    private final ModelPart left_leg;

    public CrystalSkeletonModel(ModelPart root) {
        this.root = root;
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition left_arm = body.addOrReplaceChild("left_arm", CubeListBuilder.create().texOffs(40, 16).addBox(-1.0F, -2.0F, -1.0F, 2.0F, 12.0F, 2.0F, new CubeDeformation(0.0F)), PartPose.offset(5.0F, -10.0F, 0.0F));
        ...
        PartDefinition right_leg = partdefinition.addOrReplaceChild("right_leg", CubeListBuilder.create().texOffs(0, 16).addBox(-1.0F, 0.0F, -1.0F, 2.0F, 12.0F, 2.0F, new CubeDeformation(0.0F)), PartPose.offset(-1.9F, 12.0F, 0.0F));

        PartDefinition left_leg = partdefinition.addOrReplaceChild("left_leg", CubeListBuilder.create().texOffs(0, 16).addBox(-1.0F, 0.0F, -1.0F, 2.0F, 12.0F, 2.0F, new CubeDeformation(0.0F)), PartPose.offset(1.9F, 12.0F, 0.0F));

        return LayerDefinition.create(meshdefinition, 64, 32);
    }

    @Override
    public void setupAnim(CrystalSkeleton entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        this.animate(entity.idleAnimationState, CrystalSkeletonAnimation.idle, ageInTicks);
        this.animate(entity.meleeAnimationState, CrystalSkeletonAnimation.attack, ageInTicks);
        this.animateWalk(CrystalSkeletonAnimation.walk, limbSwing, limbSwingAmount, 1.0f, 1.0f);
    }
